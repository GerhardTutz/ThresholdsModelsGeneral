...
############################################# General Fit function for fixed difficulty functions
...
ThreshModFixed <- function(dat,commonslope ,indicator=indicator, der=der,lin =lin, start, startind){
...
prodfctderiv1 <-function(theta,datitem=datitem,I=I,parmatr=parmatr,slope=slope,indicator=indicator, lin=lin,
  ...
  if(indicator=="C"){
    if(lin=="lin"){
      ...
      prod <- exp(- arg*arg/2)*arg*iteml[2]/sqrt(2*pi)
  ...
  if(indicator=="D"){
    if(lin=="log"){
      ...
      if (datitem[comp]<=0){arg2 <- theta - iteml[1]-iteml[2]*log(1+datitem[comp])
      prod <- dnorm(arg2,0,1)}  
...
deltalinNVdens <-function(y,iteml,slope , theta, lin=lin){
  ...
  for (i in 1:length(y)){
    ...
    #DFval[i,1]<-   dnorm(slope*theta - val)*iteml[2] alt war überall
    }
  return(DFval)}
#########################
...
#### wohl veraltet ohne slope in delta

############################# negative Loglikelihood function ohne stdmixt
Loglik <-function(par,dat,I,indicator, lin=lin ){
  parmatr <- matrix(par,I,2)
  slope <- 1 ### slope fixed  
  #I <- dim(dat)[1]
  P <- dim(dat)[2]
  ...
  for (p in 1:P){
    ...
    int <-gauss.hermite(prodfct, mu = 0, sd = 1,datitem=datitem,I=I,parmatr=parmatr,slope=slope,
                        indicator=indicator, lin =lin,order = 10)
    sum <- sum +log(int)
  }
  sum<- -sum
...
############################# derivative negative Loglikelihood function
derLoglik <-function(par,dat,I,indicator, lin =lin){
  ## works for lin = lin or log (difficulty functions)
  ### wohl nur für stetige und varying slopes
  ...
  for (p in 1:P){
    datitem <- dat[,p]
    ### denominator
    intn <-gauss.hermite(prodfct, mu = 0, sd = 1,datitem=datitem,I=I,parmatr=parmatr,slope=slope,
                         indicator=indicator, lin =lin,order = 10)  
    for (l in 1:I){
      comp <- l
      der[l,1] <-der[l,1]+gauss.hermite(prodfctderiv1, mu = 0, sd = 1,datitem=datitem,I=I,parmatr=parmatr,
                                        slope=slope,indicator=indicator, lin =lin,order = 10,comp=comp)/intn
    }} # end p
  ...
  for (p in 1:P){
    ...
    for (l in  1:I){
      comp <- l
      der[I+l,1] <-der[I+l,1]+gauss.hermite(prodfctderiv2, mu = 0, sd = 1,datitem=datitem,I=I,parmatr=parmatr,
                                            slope=slope,indicator=indicator, lin =lin,order = 10,comp=comp)/intn
    }} # end p
...
##################################### extended version with common item slope without stdmixt
LoglikIntExt <-function(par,dat,I,indicator, lin=lin){
  ###only intercept, common slope
  #parmatr <- matrix(par,I,2)
  # par vector intercept items, length I+1 letzter is slope
  slope <- 1 ### slope fixed  weight on theta!
  ...
  for (p in 1:P){
    datitem <- dat[,p]
    slopeitem <- par[I+1]
    int <-gauss.hermite(prodfctIntExt, mu = 0, sd = 1,datitem=datitem,I=I,par=par,slope=slope,slopeitem,
                        indicator=indicator, lin=lin,order = 10)
...
###########################
prodfctInt <-function(theta,datitem=datitem,I=I,par=par,slope=slope,indicator=indicator, lin=lin){
  ### datitem vector length I
  #####slope on y is 1
  prod <-1    
  if(indicator=="C"){
...
###########################
deltalinNV <-function(y,iteml, slope , theta){
  #### computes distribution function
  ### y vector of values
  ### iteml lineare item coefficienten 
  ### slope parameter item
  DFval <- matrix(0,length(y),1)
  for (i in 1:length(y)){
    val <- iteml[1] + iteml[2]*y
...
##################################### veraltete Version völlig ohne slope in delta
LoglikInt <-function(par,dat,I,indicator){
  ###only intercept
  #parmatr <- matrix(par,I,2)
  # par vector intercept items, length I
  slope <- 1 ### slope fixed  weight on theta!
  #I <- dim(dat)[1]
  P <- dim(dat)[2]
  sum <- 0
  for (p in 1:P){
    ...
    int <-gauss.hermite(prodfctInt, mu = 0, sd = 1,datitem=datitem,I=I,par=par,slope=slope,
                        indicator=indicator,order = 10)
...
###########################
deltalinNVold <-function(y,iteml , theta){
  #### computes distribution function
  ### y vector of values
  ### iteml lineare item coefficienten 
  DFval <- matrix(0,1,length(y))
  for (i in 1:length(y)){
    val <- iteml[1] + iteml[2]*y[i]
    DFval[1,i]<- 1 -pnorm(theta - val)}
  return(DFval)}
#########################
logist <-function(x){
