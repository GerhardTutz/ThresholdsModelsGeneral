...
#### programs for   fixed function + splines
 
ThreshFixedBasis <- function(dat,basis,splinesplus,indicator,ord, numknotsstart,lambdpenin,lambdc,start, startind,der){
  
  #### datb I x P matrix responses
  ####  basis: splines for splines, lin for linear,
  ##### splinesplus indicator of item fct if splines are chosen, only log implemented
  #     computes fixed function + splines
  ...
  ##### penalization not yet adapted
  ##### parameter basis could be dropped if only extended is estimated
  ##### loglik strange if fixed version is used (without splines)
  ...
  if(basis =="splines"){
    ...
    #### extension of Splines
    if(splinesplus =="log"){
      numknotsnow <- 2
      basnow <- array(0, dim=c(I,P,numknotsnow))
      basderivnow <- array(0, dim=c(I,P,numknotsnow))
      basdummynow <- array(0, dim=c(I,P,numknotsnow)) 
      
      for (i in 1:I){for (p in 1:P)   basnow[i,p,]<- c(1,log(1+datb[i,p]))}
      for (i in 1:I){for (p in 1:P)   basderivnow[i,p,]<- c(0,1/(1+datb[i,p]))}
      
      for (i in 1:I)if (indicator[i] !="C"){{for (p in 1:P){
        datdum <- datb[i,p]-1
        if (datdum > -1) basdummynow[i,p,]<- c(1,log(datdum+1))}}} ### discrete corrected
      
      numknotsext<-numknots+2
      basext <- array(0, dim=c(I,P,numknotsext))
      basderivext <- array(0, dim=c(I,P,numknotsext))
      basdummyext <- array(0, dim=c(I,P,numknotsext)) 
      
      for (i in 1:I){
        for (p in 1:P){basext[i,p,]<-c(basnow[i,p,],bas[i,p,])
        basderivext[i,p,]<-c(basderivnow[i,p,],bas[i,p,])
        basdummyext[i,p,]<-c(basdummynow[i,p,],basdummy[i,p,])
        }}# end i,p
      
      bas<- basext
      basderiv<-basderivext
      basdummy<-basdummyext
      numknots<-numknotsext
      
      ##################
      #parp   <- seq(1,2,1)
      #parfixed <- parp-itemsums[1]/3
      #parspl <- rep(0,numknots-2)
      #par<- c(parfixed,parspl)
      #for (l in 2:I) par <- c(par,parp-itemsums[l]/3,parspl)
      #par<-par/5
      #par <- c(par,1)
      #####
      
      par1 <- -itemsums/3  ### 
      par <- c(par1[1],1,rep(0,numknots-2)) ### 1:slope difficulty
      for(i in 2:I) par <- c(par,par1[i],2,rep(0,numknots-2))
      par <- c(par,1) ##
  ...
  if(basis =="log"){
    ...
    for (i in 1:I)if (indicator[i] !="C"){{for (p in 1:P){
      datdum <- datb[i,p]-1
      if (datdum > 0) basdummy[i,p,]<- c(1,log(datdum))}}} ### discrete not clear
...
LoglikIntExtBsplIN <-function(par,lambdpenin=lambdpenin,lambdc=lambdc,numknots=numknots,dat=dat,I=I,
  ...
  for (p in 1:P){
    ...
    ####
    #int <-gauss.hermite(prodfctIntExtBsplINVec, mu = 0, sd = sdnormal,datitem=datitem,I=I,par=par,slope=slope,
    #                    indicator=indicator, bas=bas,basderiv=basderiv,basdummy=basdummy,pers=pers,numknots=numknots)
