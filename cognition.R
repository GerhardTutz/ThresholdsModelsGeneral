...
#setwd("C:\\Users\\Gerhard Tutz\\LRZ Sync+Share\\TuRaschThresholdM\\R3\\")


############ Fits cognition data set: first fixed difficulty functions + plots
#                          later Splines + plots
...
##### cognition  data data set I x P matrix
dat <- read.table("cogn1", header=TRUE,sep= " ")


I <- dim(dat)[1]
P <- dim(dat)[2]
lin <- "lin"  ### linear difficulty function

### fit varying slopes 

Thr <- ThreshModFixed(dat,commonslope="var" ,indicator= "C", der="der",lin =lin,start, startind="no")
Thr
...
### fit fixed slopes (estimated without derivative)

# restart until Thr$convergence=0
start <- Thr$par
Thr <- ThreshModFixed(dat,commonslope="com" ,indicator= "C", der="no",lin =lin,start=start, startind="yes")
Thr


### fit with discrimination parameter

start <- Thr$par ## from Fixed fit commonslope="var"
start<-c(start,rep(1,5))
Thrab <- ThreshModFixedAlpha(dat,commonslope="var" ,indicator= "C", der="no",lin =lin,start, startind="yes",0)
Thrab 
...
Thra <- ThreshModFixedAlpha(dat,commonslope="var" ,indicator= "C", der="der",lin =lin,start, startind="yes",0)
...
PostFitV <- PosteriorEstimates(grid,dat,I,indicator,lin =lin,parmatrest,stdest=1) 
